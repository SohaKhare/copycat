...
def execute_macos_plan(
    plan: ExecutionPlan,
) -> ExecutionResult:
    """
    Execute a macOS execution plan.
...
def execute_step(
    ...
    if action == "find_folder":
        return find_folder(parameters)

    if action == "find_file":
        return find_file(parameters)

    if action == "create_folder":
        return create_folder(parameters)

    if action == "rename_folder":
        return rename_folder(parameters)

    if action == "move_file":
        return move_file(parameters)

    if action == "verify_exists":
        return verify_exists(parameters)

    raise ValueError(
        f"Unsupported macOS action: {action}"
    )
...
def rename_folder(
    ...
    if (
        ...
        and WORKSPACE
        not in destination_path.parents
    ):
        raise ValueError(
